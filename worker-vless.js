...
let userID = '########-####-####-####-############';
let proxyIP = '???.???.???.???';
let dohURL = 'https://x.xxx.x/dns-query';
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname) {
					...
					default:
						...
						newHeaders.set('cf-connecting-ip', newHeaders.get('saturnalia.pages.dev'));
						newHeaders.set('x-forwarded-for', newHeaders.get('saturnalia.pages.dev'));
						newHeaders.set('x-real-ip', newHeaders.get('saturnalia.pages.dev'));
						newHeaders.set('referer', 'https://www.bing.com/');
...
const portSet_http = new Set([80, 8080]);
const portSet_https = new Set([443, 8443]);
